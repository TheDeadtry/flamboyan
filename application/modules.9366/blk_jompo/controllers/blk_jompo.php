...
class blk_jompo extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_jompo');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_jompo->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_jompo'] 	= $this->M_blk_jompo->tampil_data_jompo($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_jompo->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_jompo->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_jompo->tampil_data_select('blk_pelajaran_jompo');
				$data['tampil_data_group'] 			= $this->M_blk_jompo->tampil_group();
				$data['count_totala'] 				= $this->M_blk_jompo->count_row('blk_penilaian_jompo',$idbio,'nilai_a_id');
				$data['count_totalb'] 				= $this->M_blk_jompo->count_row('blk_penilaian_jompo',$idbio,'nilai_b_id');
				$data['t_nilaia'] 					= $this->M_blk_jompo->sum_row('blk_penilaian_jompo',$idbio,'nilai_a_id');
				$data['t_nilaib'] 					= $this->M_blk_jompo->sum_row('blk_penilaian_jompo',$idbio,'nilai_b_id');

				$data['namamodule'] = "blk_jompo";
				$data['namafileview'] = "blk_jompo";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_jompo->tambah_nilai($data);
		redirect('blk_jompo/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_jompo->ubah_nilai($data,$id);
		redirect('blk_jompo/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_jompo->hapus_nilai();
		redirect('blk_jompo/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_jompo->tampil_group();
