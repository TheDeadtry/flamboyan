<?php
class M_upload_skuasa extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_skuasa($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-skuasa/';
	 ...
	 function hapus_foto_skuasa(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-skuasa/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_skuasa() {
        ...
        $config['upload_path'] = './assets/upload_skuasa/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_skuasa', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_skuasa($idnya){
		$sql = "SELECT * FROM upload_skuasa WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='skuasa'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_skuasa WHERE id_biodata='".$idnya."'");
	...
	function update_data_skuasa() {
		$id_skuasa =$this->input->post('id_skuasa');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_skuasa', $id_skuasa);
		$this->db->update('upload_skuasa', $data);
