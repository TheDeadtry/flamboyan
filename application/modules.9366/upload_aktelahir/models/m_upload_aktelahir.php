...
class M_upload_aktelahir extends CI_Model{
 ...
 function simpan_foto_aktelahir($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-aktelahir/';
	 ...
	 function hapus_foto_aktelahir(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-aktelahir/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_aktelahir() {
        ...
        $config['upload_path'] = './assets/upload_aktelahir/'; //path folder
		...
		$checkIfExist = $this->M_session->select_count("upload_aktelahir WHERE id_biodata='".$this->input->post('idbiodata')."'");
...
function tampil_data_personal($idnya){
	...
	function tampil_data_aktelahir($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='aktelahir'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_aktelahir WHERE id_biodata='".$idnya."'");
