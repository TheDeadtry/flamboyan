<?php
class M_upload_visapermit extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_visapermit($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-visapermit/';
	 ...
	 function hapus_foto_visapermit(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-visapermit/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_visapermit() {
        ...
        $config['upload_path'] = './assets/upload_visapermit/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_visapermit', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_visapermit($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='visapermit'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_visapermit WHERE id_biodata='".$idnya."'");
	...
	function update_data_visapermit() {
		$id_visapermit =$this->input->post('id_visapermit');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_visapermit', $id_visapermit);
		$this->db->update('upload_visapermit', $data);
