<?php
class M_upload_serkes extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_serkes($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-serkes/';
	 ...
	 function hapus_foto_serkes(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-serkes/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_serkes() {
        ...
        $config['upload_path'] = './assets/upload_serkes/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_serkes', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_serkes($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='serkes'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_serkes WHERE id_biodata='".$idnya."'");
	...
	function update_data_serkes() {
		$id_serkes =$this->input->post('id_serkes');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_serkes', $id_serkes);
		$this->db->update('upload_serkes', $data);
