...
class upload_spik extends MX_Controller{
	...
	$session = $this->M_session->get_session();
		...
		$status = $session['session_status'];
			...
			if ($id_user && $status==1){
				...
				$data['hitungan'] = $this->M_upload_spik->hitungan($detailidnya);
				$data['ambildatafoto'] = $this->M_upload_spik->ambildatafoto($detailidnya);
				...
				$data['namamodule'] = "upload_spik";
				$data['namafileview'] = "spikadmin";
				echo Modules::run('template/spik_template', $data);
			}
		}	 
	}
	...
	function proses_upload(){
		$detailidnya=$this->session->userdata("detailuser");
	$this->M_upload_spik->simpan_foto_spik($detailidnya);
	}


	//Untuk menghapus foto
	function remove_foto(){
$this->M_upload_spik->hapus_foto_spik();
	}
