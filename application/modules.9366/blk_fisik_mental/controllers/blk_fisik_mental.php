...
class blk_fisik_mental extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_fisik_mental');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_fisik_mental->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_fisik_mental'] 	= $this->M_blk_fisik_mental->tampil_data_fisik_mental($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_fisik_mental->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_fisik_mental->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_fisik_mental->tampil_data_select('blk_pelajaran_fisik_mental');
			 	$data['tampil_data_blk_minggu'] 	= $this->M_blk_fisik_mental->tampil_data_select('blk_minggu');
				$data['tampil_data_group'] 			= $this->M_blk_fisik_mental->tampil_group();
				$data['count_total'] 				= $this->M_blk_fisik_mental->count_row('blk_penilaian_fisik_mental',$idbio,'nilai_id');
				$data['t_nilai'] 					= $this->M_blk_fisik_mental->sum_row('blk_penilaian_fisik_mental',$idbio,'nilai_id');

				$data['namamodule'] = "blk_fisik_mental";
				$data['namafileview'] = "blk_fisik_mental";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_fisik_mental->tambah_nilai($data);
		redirect('blk_fisik_mental/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_fisik_mental->ubah_nilai($data,$id);
		redirect('blk_fisik_mental/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_fisik_mental->hapus_nilai();
		redirect('blk_fisik_mental/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_fisik_mental->tampil_group();
