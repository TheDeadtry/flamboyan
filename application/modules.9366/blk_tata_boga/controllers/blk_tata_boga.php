...
class blk_tata_boga extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_tata_boga');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_tata_boga->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_tata_boga'] 	= $this->M_blk_tata_boga->tampil_data_tata_boga($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_tata_boga->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_tata_boga->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_tata_boga->tampil_data_select('blk_pelajaran_tata_boga');
				$data['tampil_data_group'] 			= $this->M_blk_tata_boga->tampil_group();
				$data['count_totala'] 				= $this->M_blk_tata_boga->count_row('blk_penilaian_tata_boga',$idbio,'nilai_a_id');
				$data['count_totalb'] 				= $this->M_blk_tata_boga->count_row('blk_penilaian_tata_boga',$idbio,'nilai_b_id');
				$data['t_nilaia'] 					= $this->M_blk_tata_boga->sum_row('blk_penilaian_tata_boga',$idbio,'nilai_a_id');
				$data['t_nilaib'] 					= $this->M_blk_tata_boga->sum_row('blk_penilaian_tata_boga',$idbio,'nilai_b_id');

				$data['namamodule'] = "blk_tata_boga";
				$data['namafileview'] = "blk_tata_boga";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_tata_boga->tambah_nilai($data);
		redirect('blk_tata_boga/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_tata_boga->ubah_nilai($data,$id);
		redirect('blk_tata_boga/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_tata_boga->hapus_nilai();
		redirect('blk_tata_boga/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_tata_boga->tampil_group();
