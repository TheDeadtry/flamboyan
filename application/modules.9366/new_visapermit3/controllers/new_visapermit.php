...
class new_visapermit extends MX_Controller{
	    ...
	    foreach ($tampil_data_vp as $row) {
            ...
            array_push($data2,
                array(
                    ...
                    '<a target="_blank" href="'.base_url().'/assets/dokvisapermit/'.$row->filevisapermit.'"'.$row->filevisapermit.'><img width="50px" height="50px" src="'.base_url().'assets/dokvisapermit/'.$row->filevisapermit.'"/></a>',
                    $row->no_suhan,
                    $row->no_visapermit,
                    $row->namagroupnya,
                    $row->namaagen,
