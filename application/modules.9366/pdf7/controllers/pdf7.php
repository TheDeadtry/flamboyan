...
class pdf7 extends MX_Controller{
...
$htmltodoc->createDoc($html, "pkj.doc", true); 
	...
	function cetakbaru($idbiodata) {
		require_once 'assets/phpword/PHPWord.php';
			...
			$penerbangan1=$this->m_pdf7->penerbangan1($idbiodata);
			$penerbangan2=$this->m_pdf7->penerbangan2($idbiodata);
			$maskapai=$this->m_pdf7->maskapai($idbiodata);
			...
			$bankpribadi= $this->m_pdf7->bankpribadi($idbiodata);
			$bankkarantina= $this->m_pdf7->bankkarantina($idbiodata);
			$fotopaspor= $this->m_pdf7->fotopaspor($idbiodata);
			$fotopaspor2= $this->m_pdf7->fotopaspor2($idbiodata);
			$hitungpaspor= $this->m_pdf7->hitungpaspor($idbiodata);
			$jeniskelamin= $this->m_pdf7->jeniskelamin($idbiodata);
			$adh1= $this->m_pdf7->adh1($idbiodata);
			$adh2= $this->m_pdf7->adh2($idbiodata);
			$adh3= $this->m_pdf7->adh3($idbiodata);

			$PHPWord = new PHPWord();

			$document = $PHPWord->loadTemplate('files/lt.docx');

			$document->setValue('zz1', $idtki.' '.$namatki);
			$document->setValue('zz2', '('.$adh1.')');
			$document->setValue('zz3', 'LINE ID : '.$adh2);
			$document->setValue('zz4', 'EMAIL : '.$adh3);
			$document->setValue('karantina', $bankkarantina);
			...
			$ambilservak = "SELECT * FROM foto WHERE id_biodata='$idbiodata' AND jenis='servak'";
			$count = $this->db->query($ambilservak)->result();
			$document->cloneRow('vaksin1',count($count));
			$document->setValue('vaksin2', '');
			foreach($count as $kk => $vv)
			{
				$no = $kk+1;
				$dataktpsaya="_upload/vaksin/".$vv->namafile;
				$aImgs7 = array(
					array(
						'img' => $dataktpsaya,
						'size' => array(1000, 600),
				...
				$document->replaceStrToImg( 'vaksin1#'.$no, $aImgs7);
			}
...
/*
			...
			if($count == 1){
				...
				$namaktp = $this->db->query($ambilservak)->row()->namafile;
				$dataktpsaya="_upload/vaksin/".$namaktp;
				$aImgs7 = array(
					array(
						'img' => $dataktpsaya,
						'size' => array(1000, 600),
				...
				$document->replaceStrToImg( 'vaksin1', $aImgs7);
				$document->setValue('vaksin2', '');
			}
			if($count > 1){
				$namaktp1 = $this->db->query($ambilservak.' ORDER BY id ASC LIMIT 1')->row()->namafile;
				$namaktp2 = $this->db->query($ambilservak.' ORDER BY id DESC LIMIT 1')->row()->namafile;
				$dd1="_upload/vaksin/".$namaktp1;
				$dd2="_upload/vaksin/".$namaktp2;
				$ddd1 = array(
					array(
						'img' => $dd1,
						'size' => array(1000, 600),
				...
				$document->replaceStrToImg( 'vaksin1', $ddd1);
				$document->replaceStrToImg( 'vaksin2', $ddd2);
			}*/
			...
			$maskapai = str_replace(' ','-',$maskapai);
			$xxx = strpos($maskapai,'-');
			$maskapai = substr($maskapai,0,$xxx);

			$qq1 = mb_substr($namamajikantaiwan, 0 , 2, 'utf-8').' '.$tanggaltiba.' '.$idtki.' '.$namatki.' '.$maskapai.'.docx';
			...
			header('Content-Disposition: attachment; filename='.$qq1);
...
exit;
	...
	function cetaklaporansuhanvp() {
		...
		$idagen	= $this->input->post('id_agen');
		if ($idagen == 'x_semua_agen') {
			redirect('pdf7/cetaklaporansuhanvp_semua');
				}
		$idsuhan	= $this->input->post('id_suhan');
		if ($idagen == 'x_semua_suhan') {
			redirect('pdf7/cetaklaporansuhanvp_semua');
				}
		$tampidatanyalaporanagen=$this->m_pdf7->tampidatanyalaporanagen($idagen);
		$hitungdatanyaagen=$this->m_pdf7->hitungdatanyaagen($idagen);
		...
		$document = $PHPWord->loadTemplate('files/laporansuhanvpdiindo.docx');
		...
		foreach($tampidatanyalaporanagen->result() as $row) {
		$document->setValue('value4#'.$no,$no);
		...
		// $document->setValue('value8#'.$no, $row->id_biodata_dititipkan_sh."".$row->nama_dititipkan_sh."".$row->tgl_terbang_dititipkan_sh);
		// $document->setValue('value8#'.$no, $row->id_biodata_dititipkan_sh."".$row->nama_dititipkan_sh."".$row->tgl_terbang_dititipkan_sh);

		$document->setValue('value33#'.$no,$row->tanggalterbang);
		...
		$document->setValue('value15#'.$no,$row->tempat_sh);
		$document->setValue('value16#'.$no,$row->tempat_vp);

		$document->setValue('value99#'.$no,$row->status_sh);
		$document->setValue('value98#'.$no,$row->status_vp);
		
		// $document->setValue('value22#'.$no,$row->suhanket);
		// $document->setValue('value23#'.$no,$row->visapermitket);

		
		$document->setValue('value24#'.$no,$row->idbiodititipsh);
		$document->setValue('value25#'.$no,$row->namadititipsh);
		$document->setValue('value26#'.$no,$row->tgldititipsh);
		
		// $document->setValue('value27#'.$no,$row->idbiodititipvp);
		// $document->setValue('value28#'.$no,$row->namadititipsvp);
		// $document->setValue('value29#'.$no,$row->tgldititipvp);
		
		$document->setValue('value30#'.$no,$row->idbiotitip);
		$document->setValue('value31#'.$no,$row->namatitip);
		$document->setValue('value32#'.$no,$row->tgltitip);
...
function Terbilang($x)
{
 ...
 //	foreach($tampil_data_ctkinya_pengajuan_ujk->result() as $rows) {
	...
	function cetaklaporanperwilayahdisnaker() {
		$tglmulai	= $this->input->post('tglmulai');
		$tglakhir	= $this->input->post('tglakhir');

		$wildis	= $this->input->post('wildis'); //wilayah disnaker
		if ($wildis == '') {
			exit('belum diisi wilayah disnaker');
		}
		$tampidatanyaagen=$this->m_pdf7->tampidatanyaagen($tglmulai,$tglakhir,$idagen);
		$hitungdatanyaagen=$this->m_pdf7->hitungdatanyaagen($tglmulai,$tglakhir,$idagen);
