...
class Detaildisnaker extends MX_Controller{
	...
	$session = $this->M_session->get_session();
		...
		$status = $session['session_status'];
			...
			if ($id_user && $status==1){
				...
				$data['tampil_data_setting_pendidikan'] = $this->M_session->select("SELECT * FROM setting_pendidikan");
				$data['tampil_data_agen'] = $this->M_detaildisnaker->tampil_data_agen();
				$data['tampil_data_gaji'] = $this->M_detaildisnaker->tampil_data_gaji();
				$data['tampil_data_hubungan'] = $this->M_detaildisnaker->tampil_data_hubungan();
				...
				$data['data_notelp'] = $this->M_session->getDataWhere("notelp","personal","id_biodata='".$this->session->userdata("detailuser")."'");
				$data['tampil_data_family'] = $this->M_detaildisnaker->tampil_data_family($this->session->userdata("detailuser"));
				$data['tampil_data_namadisnaker'] = $this->M_session->select("SELECT * FROM datanamadisnaker");

				$data['hitungdisnaker'] = $this->M_detaildisnaker->hitung_data_disnaker($this->session->userdata("detailuser"));
				$tglx= date("d/m/Y");
				$data['tglxss'] = $tglx;

				$data['tampil_data_registrasi'] = $this->M_session->select("SELECT * FROM dataregdisnaker order by nama");
...
if (isset($_POST['resetid'])) {
	...
	function updatelegal() {
		...
		$this->M_detaildisnaker->update_keluarga();
		redirect('detaildisnaker');
	}
