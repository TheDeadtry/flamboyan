...
class M_upload_visa extends CI_Model{
 ...
 function simpan_foto_visa($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-visa/';
	 ...
	 function hapus_foto_visa(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-visa/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_visa() {
        ...
        $config['upload_path'] = './assets/upload_visa/'; //path folder
...
function tampil_data_personal($idnya){
	...
	function tampil_data_visa($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='visa'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_visa WHERE id_biodata='".$idnya."'");
