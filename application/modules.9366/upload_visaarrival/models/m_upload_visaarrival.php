<?php
class M_upload_visaarrival extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_visaarrival($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-visaarrival/';
	 ...
	 function hapus_foto_visaarrival(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-visaarrival/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_visaarrival() {
        ...
        $config['upload_path'] = './assets/upload_visaarrival/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_visaarrival', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_visaarrival($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='visaarrival'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_visaarrival WHERE id_biodata='".$idnya."'");
	...
	function update_data_visaarrival() {
		$id_visaarrival =$this->input->post('id_visaarrival');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_visaarrival', $id_visaarrival);
		$this->db->update('upload_visaarrival', $data);
