<?php
class M_blk_bhs_taiyu extends CI_Model{
    function __construct(){
        parent::__construct();
    }
 
	function tampil_data_bhs_taiyu($tgl,$nodaftar) {
		$sql = "SELECT a.id_penilaian_bhs_taiyu as idpgl, 
		a.keterangan as ket,d.kode_materi as kode, a.tgl as tgl, b.id_instruktur, d.id_bhs_taiyu,
		d.nama_materi as nama,b.nama as instruktur,a.nilai_a_id as nt,a.nilai_b_id as np, b.kode_instruktur as kodinst
		from blk_penilaian_bhs_taiyu a JOIN blk_instruktur b JOIN blk_pelajaran_bhs_taiyu d
		where a.penilai_id=b.id_instruktur and a.bhs_taiyu_id=d.id_bhs_taiyu and a.no_daftar='$nodaftar' and a.tgl='$tgl' and tipe=1 
	...
	/* -- BACKUP
	function tampil_data_bhs_taiyu($nodaftar) {
		$sql = "SELECT a.id_penilaian_bhs_taiyu as idpgl,
		...
		from blk_penilaian_bhs_taiyu a JOIN blk_instruktur b JOIN blk_pelajaran_bhs_taiyu d 
		where a.penilai_id=b.id_instruktur and a.bhs_taiyu_id=d.id_bhs_taiyu and a.no_daftar='$nodaftar' order by a.tgl desc, a.penilai_id desc";
        $query = $this->db->query($sql);
	    return $query->result();
	} */
	...
	function tambah_nilai($data) {
		$this->db->insert('blk_penilaian_bhs_taiyu',$data);
	}

	function tampil_group() {
		$sql = "SELECT MIN(id_penilaian_bhs_taiyu) AS id, tgl
		FROM blk_penilaian_bhs_taiyu
		WHERE tipe=1
	...
	function tampil_data_count($tgl,$id) {
		...
		from blk_penilaian_bhs_taiyu a JOIN blk_instruktur b JOIN blk_pelajaran_bhs_taiyu d 
		where a.penilai_id=b.id_instruktur and a.bhs_taiyu_id=d.id_bhs_taiyu 
	...
	function ubah_nilai($data,$id) {
		$this->db->where('id_penilaian_bhs_taiyu', $id);
		$this->db->update('blk_penilaian_bhs_taiyu', $data);
	}

	function hapus_nilai() {
		$id = $this->input->post('tatagraha_id');
		$this->db->where('id_penilaian_bhs_taiyu', $id);
		$this->db->delete('blk_penilaian_bhs_taiyu');
	}

	function ambil_kali($field,$nodaftar,$materi) {
		$sql = "SELECT count(*) as kali FROM blk_penilaian_bhs_taiyu where tipe=1 and no_daftar='$nodaftar' and bhs_taiyu_id='$materi' and '$field' IS NOT NULL";
		$q = $this->db->query($sql);
		return $q->row();
	}
	...
	function ambil_rata($nodaftar,$materi,$field_id) {
		$a = $field_id.'_id';
		$sql = "SELECT avg(b.nilai) as nilai FROM blk_penilaian_bhs_taiyu a JOIN blk_nilai b where a.$a=b.id_nilai and a.no_daftar='$nodaftar' and a.bhs_taiyu_id='$materi' and a.$a IS NOT NULL";
		$q = $this->db->query($sql);
		return $q->row();
	}
