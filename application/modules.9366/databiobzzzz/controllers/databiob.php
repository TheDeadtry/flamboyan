...
class Databiob extends MX_Controller{
  	...
  	public function detaildata($user_id) {
   	...
   	/*
		...
		for($i=0;$i<count($idbio);$i++){
			...
			$lengthNumb[] = $this->M_databiob->datatables_count_where($table, $primaryKey, $where_personal[$i]);
		}
		...
		$resTotalLength =  $this->M_databiob->datatables_count($table, $primaryKey, $pilsek);
        ...
        for($i=0;$i<count($idper);$i++) { 
			...
			array_push($data2,
				array(
					...
					$personal[$i][2],
					$personal[$i][3],
					$personal[$i][4].'/'.$personal[$i][5],
					$personal[$i][12],
					$personal[$i][6],
					$personal[$i][7],
					$personal[$i][8],
					$personal[$i][9],
					$personal[$i][10].' / '.$personal[$i][11],
					$personal[$i][13],
					'',
					'',
					'',
