<?php
class M_upload_waris extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_waris($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-waris/';
	 ...
	 function hapus_foto_waris(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-waris/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_waris() {
        ...
        $config['upload_path'] = './assets/upload_waris/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_waris', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_waris($idnya){
		$sql = "SELECT * FROM upload_waris WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='waris'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_waris WHERE id_biodata='".$idnya."'");
	...
	function update_data_waris() {
		$id_waris =$this->input->post('id_waris');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_waris', $id_waris);
		$this->db->update('upload_waris', $data);
