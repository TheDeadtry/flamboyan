<?php
class M_upload_perjanjianpenempatan extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_perjanjianpenempatan($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-perjanjianpenempatan/';
	 ...
	 function hapus_foto_perjanjianpenempatan(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-perjanjianpenempatan/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_perjanjianpenempatan() {
        ...
        $config['upload_path'] = './assets/upload_perjanjianpenempatan/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_perjanjianpenempatan', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_perjanjianpenempatan($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='perjanjianpenempatan'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_perjanjianpenempatan WHERE id_biodata='".$idnya."'");
	...
	function update_data_perjanjianpenempatan() {
		$id_perjanjianpenempatan =$this->input->post('id_perjanjianpenempatan');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_perjanjianpenempatan', $id_perjanjianpenempatan);
		$this->db->update('upload_perjanjianpenempatan', $data);
