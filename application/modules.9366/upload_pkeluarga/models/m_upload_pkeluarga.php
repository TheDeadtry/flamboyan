...
class M_upload_pkeluarga extends CI_Model{
 ...
 function simpan_foto_pkeluarga($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-pkeluarga/';
	 ...
	 function hapus_foto_pkeluarga(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-pkeluarga/'.$nama_foto)){
				unlink($file);
			}
...
function tampil_data_personal($idnya){
	...
	function tampil_data_pkeluarga($idnya){
		$sql = "SELECT * FROM upload_pkeluarga WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='pkeluarga'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_pkeluarga WHERE id_biodata='".$idnya."'");
