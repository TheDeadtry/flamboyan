<?php
class M_upload_job extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_job($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-job/';
	 ...
	 function hapus_foto_job(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-job/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_job() {
        ...
        $config['upload_path'] = './assets/upload_job/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_job', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_job($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='job'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_job WHERE id_biodata='".$idnya."'");
	...
	function update_data_job() {
		...
		$this->db->update('upload_job', $data);
