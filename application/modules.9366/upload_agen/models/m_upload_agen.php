<?php
class M_upload_agen extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_agen($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-agen/';
	 ...
	 function hapus_foto_agen(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-agen/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_agen() {
        ...
        $config['upload_path'] = './assets/upload_agen/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_agen', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_agen($idnya){
		$sql = "SELECT * FROM upload_agen WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='agen'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_agen WHERE id_biodata='".$idnya."'");
	...
	function update_data_agen() {
		...
		$this->db->update('upload_agen', $data);
