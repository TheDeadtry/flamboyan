<?php
class M_upload_skck extends CI_Model{
 ...
 function simpan_foto_skck($idbio){
	...
	function simpan_data_skck() {
        ...
        $config['upload_path'] = './assets/upload_skck/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_skck', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_skck($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='skck'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_skck WHERE id_biodata='".$idnya."'");
	...
	function update_data_skck() {
		...
		$this->db->update('upload_skck', $data);
