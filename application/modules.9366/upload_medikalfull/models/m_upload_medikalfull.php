<?php
class M_upload_medikalfull extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_medikalfull($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-medikalfull/';
	 ...
	 function hapus_foto_medikalfull(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-medikalfull/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_medikalfull() {
        ...
        $config['upload_path'] = './assets/upload_medikalfull/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_medikalfull', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_medikalfull($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='medikalfull'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_medikalfull WHERE id_biodata='".$idnya."'");
	...
	function update_data_medikalfull() {
		$id_medikalfull =$this->input->post('id_medikalfull');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_medikalfull', $id_medikalfull);
		$this->db->update('upload_medikalfull', $data);
