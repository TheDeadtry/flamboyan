<?php
class M_baru4 extends CI_Model{
    function __construct(){
        parent::__construct();
    }
function tampil_namanya($id_siswa){
$kode_desa="";
        $result = Dbs::load("SELECT * FROM format_disnaker_formal
        LEFT JOIN personal
        ON format_disnaker_formal.id_biodata=personal.id_biodata 
WHERE format_disnaker_formal.id_biodata='$id_siswa'");
...
$kode_desa="";
        $result = Dbs::load("SELECT * FROM format_disnaker_formal
        LEFT JOIN family
        ON format_disnaker_formal.id_biodata=family.id_biodata 
WHERE format_disnaker_formal.id_biodata='$id_siswa'");
			while($row = mysqli_fetch_array($result)){
...
$kode_desa="";
        $result = Dbs::load("SELECT * FROM format_disnaker_formal
        LEFT JOIN paspor
        ON format_disnaker_formal.id_biodata=paspor.id_biodata 
WHERE format_disnaker_formal.id_biodata='$id_siswa'");
			while($row = mysqli_fetch_array($result)){
