...
class Detailasuransi extends MX_Controller{
		...
		$status = $session['session_status'];
			...
			if ($id_user && $status==1){
				...
				$data['tampil_data_provinsi'] = $this->M_detailasuransi->tampil_data_provinsi();
								$data['tampil_pil_asuransi'] = $this->M_detailasuransi->tampil_pil_asuransi();
								$data['tampil_data_namaasuransi'] = $this->M_detailasuransi->tampil_data_namaasuransi();

				$data['detailpersonalid'] = $this->session->userdata("detailuser");
				$data['tampil_data_asuransi'] = $this->M_detailasuransi->tampil_data_asuransi($this->session->userdata("detailuser"));
				$data['tampil_data_asuransifull'] = $this->M_detailasuransi->tampil_data_asuransifull($this->session->userdata("detailuser"));
				...
				$data['hitungasuransifull'] = $this->M_detailasuransi->hitung_data_asuransifull($this->session->userdata("detailuser"));
...
$this->M_detailasuransi->tambahasuransifull();

		redirect('detailasuransi');
		}
			function ubahasuransifull() {

$this->M_detailasuransi->ubahasuransifull();
