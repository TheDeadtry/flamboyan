<?php
class M_upload_kk extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_kk($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-kk/';
	 ...
	 function hapus_foto_kk(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-kk/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_kk() {
        ...
        $config['upload_path'] = './assets/upload_kk/'; //path folder
...
function tampil_data_personal($idnya){
	...
	function tampil_data_kk($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='kk'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_kk WHERE id_biodata='".$idnya."'");
