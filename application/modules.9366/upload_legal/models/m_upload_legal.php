<?php
class M_upload_legal extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_legal($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-legal/';
	 ...
	 function hapus_foto_legal(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-legal/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_legal() {
        ...
        $config['upload_path'] = './assets/upload_legal/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_legal', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_legal($idnya){
		$sql = "SELECT * FROM upload_legal WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='legal'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_legal WHERE id_biodata='".$idnya."'");
	...
	function update_data_legal() {
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_legal', $id_legal);
		$this->db->update('upload_legal', $data);
