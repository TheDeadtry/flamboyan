...
class M_upload_ktp extends CI_Model{
 ...
 function simpan_foto_ktp($idbio){
	 ...
	 function hapus_foto_ktp(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-ktp/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_ktp() {
        ...
        $config['upload_path'] = './assets/upload_ktp/'; //path folder
...
function tampil_data_personal($idnya){
	...
	function tampil_data_ktp($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='ktp'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_ktp WHERE id_biodata='".$idnya."'");
