...
class M_upload_pk extends CI_Model{
 ...
 function simpan_foto_pk($idbio){
        ...
        $config['upload_path']   = FCPATH.'/upload-pk/';
	 ...
	 function hapus_foto_pk(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-pk/'.$nama_foto)){
				unlink($file);
			}
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_pk WHERE id_biodata='".$idnya."'");
