<?php
class M_surat_rekom_tabeldis3 extends CI_Model{
    function __construct(){
        parent::__construct();
    }
 
 function tampil_data_personal(){
		$sql = "SELECT *,pembuatan_tabeldis3.id_pembuatan as idnyabuat FROM pembuatan_tabeldis3 ";
     ...
     function tampil_data_personalcari($id){
        $sql = "SELECT *,pembuatan_tabeldis3.id_pembuatan as idnyabuat FROM pembuatan_tabeldis3 JOIN detail_tabeldis3 ON pembuatan_tabeldis3.id_pembuatan = detail_tabeldis3.id_tabeldis3 where detail_tabeldis3.id_biodata='$id'
                ";
                $query = $this->db->query($sql);
 ...
 function tampil_data_tki(){
    ...
    function tampil_data_namadisnaker(){
        ...
        $sql = "SELECT * FROM detail_tabeldis3 
LEFT JOIN personal
ON personal.id_biodata=detail_tabeldis3.id_biodata
where detail_tabeldis3.id_tabeldis3='$idtabeldis'";
                $query = $this->db->query($sql);
 ...
 function simpan_data_surat(){
        ...
        $this->db->insert('pembuatan_tabeldis3',$data);
    }
...
$daerah = $this->input->post('daerah');
        ...
        $this->db->update('pembuatan_tabeldis3',$data);
    }

    function hapus_data_surat() {
        $id_pembuatan = $this->input->post('id_pembuatan');
        $this->db->where('id_pembuatan', $id_pembuatan);
        $this->db->delete('pembuatan_tabeldis3');
    }
      ...
      function simpan_data_ctki(){
        ...
        $data = array (
            'id_tabeldis3'=>$id_pembuatan,
            'id_biodata'=> $idsimpan,
            );
        $this->db->insert('detail_tabeldis3',$data);
    }
    ...
    function update_ctki() {
        ...
        $data = array (
            'id_biodata'=> $idsimpan,
             ...
             $this->db->where('id_pembuatan', $id_pembuatan);
        $this->db->update('detail_tabeldis3', $data);
    }
        function hapus_ctki() {
        $id = $this->input->post('id_pembuatan');
        $this->db->where('id_pembuatan', $id);
        $this->db->delete('detail_tabeldis3');
    }
