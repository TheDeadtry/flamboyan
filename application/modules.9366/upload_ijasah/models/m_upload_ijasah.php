<?php
class M_upload_ijasah extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_ijasah($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-ijasah/';
	 ...
	 function hapus_foto_ijasah(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-ijasah/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_ijasah() {
        ...
        $config['upload_path'] = './assets/upload_ijasah/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_ijasah', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_ijasah($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='ijasah'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_ijasah WHERE id_biodata='".$idnya."'");
	...
	function update_data_ijasah() {
		$id_ijasah =$this->input->post('id_ijasah');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_ijasah', $id_ijasah);
		$this->db->update('upload_ijasah', $data);
