<?php
class M_upload_devisapermit extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_devisapermit($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-devisapermit/';
	 ...
	 function hapus_foto_devisapermit(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-devisapermit/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_devisapermit() {
        ...
        $config['upload_path'] = './assets/upload_devisapermit/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_devisapermit', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_devisapermit($idnya){
		$sql = "SELECT * FROM upload_devisapermit WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='devisapermit'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_devisapermit WHERE id_biodata='".$idnya."'");
	...
	function update_data_devisapermit() {
		$id_devisapermit =$this->input->post('id_devisapermit');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_devisapermit', $id_devisapermit);
		$this->db->update('upload_devisapermit', $data);
