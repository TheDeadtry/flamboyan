...
class blk_jadwal2 extends MX_Controller {
	...
	function add_hari($id) {
	 	...
	 	$setting_nama_paket					= $this->M_session->select_row("SELECT b.nama_paket FROM blk_jadwal_paket a JOIN blk_setting_paket b where a.id_paket='".$id."'");
	 	$nama_tabel 						= $setting_nama_paket->nama_paket;
	 	$data['izpaket']					= $setting_nama_paket->nama_paket;


		$query = "
			SELECT a.kode_materi,a.nama_materi,concat(a.kode_materi,'|-|berhitung') as id FROM $nama_tabel a";
			/*
		$query = "
...
/*		
		...
		*/
		...
		//===================
		$jadwal_ambil_id = $this->M_session->select_row("SELECT * FROM blk_jadwal_paketjadwal WHERE hari='$hari' AND jam='$jam' ORDER BY id_jadwal_paket_jadwal DESC");
		//==================
		$data['v_id'] 	= $v_id;
