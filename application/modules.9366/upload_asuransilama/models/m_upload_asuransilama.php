<?php
class M_upload_asuransilama extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_asuransilama($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-asuransilama/';
	 ...
	 function hapus_foto_asuransilama(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-asuransilama/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_asuransilama() {
        ...
        $config['upload_path'] = './assets/upload_asuransilama/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_asuransilama', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_asuransilama($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='asuransilama'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_asuransilama WHERE id_biodata='".$idnya."'");
	...
	function update_data_asuransilama() {
		$id_asuransilama =$this->input->post('id_asuransilama');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_asuransilama', $id_asuransilama);
		$this->db->update('upload_asuransilama', $data);
