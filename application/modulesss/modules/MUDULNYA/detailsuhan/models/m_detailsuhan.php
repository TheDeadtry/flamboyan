<?php
class M_detailsuhan extends CI_Model{
 ...
 function tampil_data_negara(){
	...
	function hitung_data_suhan($idnya){
		$sql = "SELECT count(*) as total FROM suhan WHERE id_biodata='".$idnya."'";
		...
		$sql = "SELECT * FROM suhan WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);

            return $query->result();
	}

	function tambahsuhan() {
