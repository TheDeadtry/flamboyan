<?php
class M_upload_medikal extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_medikal($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-medikal/';
	 ...
	 function hapus_foto_medikal(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-medikal/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_medikal() {
        ...
        $config['upload_path'] = './assets/upload_medikal/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_medikal', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_medikal($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='medikal'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_medikal WHERE id_biodata='".$idnya."'");
	...
	function update_data_medikal() {
		$id_medikal =$this->input->post('id_medikal');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_medikal', $id_medikal);
		$this->db->update('upload_medikal', $data);
