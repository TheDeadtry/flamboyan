<?php
class M_upload_sppf extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_sppf($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-sppf/';
	 ...
	 function hapus_foto_sppf(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-sppf/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_sppf() {
        ...
        $config['upload_path'] = './assets/upload_sppf/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_sppf', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_sppf($idnya){
		$sql = "SELECT * FROM upload_sppf WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='sppf'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_sppf WHERE id_biodata='".$idnya."'");
	...
	function update_data_sppf() {
		$id_sppf =$this->input->post('id_sppf');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_sppf', $id_sppf);
		$this->db->update('upload_sppf', $data);
