<?php
class M_upload_sertifikatujian extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_sertifikatujian($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-sertifikatujian/';
	 ...
	 function hapus_foto_sertifikatujian(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-sertifikatujian/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_sertifikatujian() {
        ...
        $config['upload_path'] = './assets/upload_sertifikatujian/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_sertifikatujian', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_sertifikatujian($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='sertifikatujian'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_sertifikatujian WHERE id_biodata='".$idnya."'");
	...
	function update_data_sertifikatujian() {
		$id_sertifikatujian =$this->input->post('id_sertifikatujian');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_sertifikatujian', $id_sertifikatujian);
		$this->db->update('upload_sertifikatujian', $data);
