...
class M_upload_spaw extends CI_Model{
 ...
 function simpan_foto_spaw($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-spaw/';
	 ...
	 function hapus_foto_spaw(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-spaw/'.$nama_foto)){
				unlink($file);
			}
...
function tampil_data_personal($idnya){
	...
	function tampil_data_spaw($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='spaw'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_spaw WHERE id_biodata='".$idnya."'");
