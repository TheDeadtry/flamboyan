...
class blk_bhs_taiyu extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_bhs_taiyu');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_bhs_taiyu->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_bhs_taiyu'] 	= $this->M_blk_bhs_taiyu->tampil_data_bhs_taiyu($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_bhs_taiyu->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_bhs_taiyu->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_bhs_taiyu->tampil_data_select('blk_pelajaran_bhs_taiyu');
				$data['tampil_data_group'] 			= $this->M_blk_bhs_taiyu->tampil_group();
				$data['count_totala'] 				= $this->M_blk_bhs_taiyu->count_row('blk_penilaian_bhs_taiyu',$idbio,'nilai_a_id');
				$data['count_totalb'] 				= $this->M_blk_bhs_taiyu->count_row('blk_penilaian_bhs_taiyu',$idbio,'nilai_b_id');
				$data['t_nilaia'] 					= $this->M_blk_bhs_taiyu->sum_row('blk_penilaian_bhs_taiyu',$idbio,'nilai_a_id');
				$data['t_nilaib'] 					= $this->M_blk_bhs_taiyu->sum_row('blk_penilaian_bhs_taiyu',$idbio,'nilai_b_id');

				$data['namamodule'] = "blk_bhs_taiyu";
				$data['namafileview'] = "blk_bhs_taiyu";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_bhs_taiyu->tambah_nilai($data);
		redirect('blk_bhs_taiyu/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_bhs_taiyu->ubah_nilai($data,$id);
		redirect('blk_bhs_taiyu/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_bhs_taiyu->hapus_nilai();
		redirect('blk_bhs_taiyu/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_bhs_taiyu->tampil_group();
