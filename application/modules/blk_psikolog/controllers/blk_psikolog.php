...
class blk_psikolog extends MX_Controller{
	    ...
	    foreach ($tampil_data as $row) 
	    {
	    	if ( $prevVal != $row->id )
	    	{
	            ...
	            array_push($data2,
	                array(
	                    $no,
	                    '<a class="btn btn-xs bg-orange" href="'.site_url('blk_psikolog/cetaks/'.$row->id).'">PRINT</a> '.
	...
	function psikolog_nilai_show() 
	{
	    ...
	    foreach ( $ambil_pmi as $row ) 
	    {
	    	...
	    	$data2 .= "<td width='20%'>
	    				...
	    				"</td>";
	    	$data2 .= '<td style="padding:0px" width="32%" height="300px">
	    	...
	    	$data2 .= '<td style="padding:0px" height="300px">
				            <div style="height:100% !important">
				                <textarea name="iq[]" style="resize:none; height:100% !important; width:100% !important" data-ng-model="modelLogin.inpMtext">'.$row->nilai.'</textarea>
				            </div>
				        </td>';
	    	$data2 .= "</tr>";
