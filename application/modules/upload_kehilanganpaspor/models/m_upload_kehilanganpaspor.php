<?php
class M_upload_kehilanganpaspor extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_kehilanganpaspor($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-kehilanganpaspor/';
	 ...
	 function hapus_foto_kehilanganpaspor(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-kehilanganpaspor/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_kehilanganpaspor() {
        ...
        $config['upload_path'] = './assets/upload_kehilanganpaspor/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_kehilanganpaspor', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_kehilanganpaspor($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='kehilanganpaspor'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_kehilanganpaspor WHERE id_biodata='".$idnya."'");
	...
	function update_data_kehilanganpaspor() {
		$id_kehilanganpaspor =$this->input->post('id_kehilanganpaspor');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_kehilanganpaspor', $id_kehilanganpaspor);
		$this->db->update('upload_kehilanganpaspor', $data);
