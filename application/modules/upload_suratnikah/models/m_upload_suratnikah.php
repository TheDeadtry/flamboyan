<?php
class M_upload_suratnikah extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_suratnikah($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-suratnikah/';
	 ...
	 function hapus_foto_suratnikah(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-suratnikah/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_suratnikah() {
        ...
        $config['upload_path'] = './assets/upload_suratnikah/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_suratnikah', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_suratnikah($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='suratnikah'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_suratnikah WHERE id_biodata='".$idnya."'");
	...
	function update_data_suratnikah() {
		$id_suratnikah =$this->input->post('id_suratnikah');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_suratnikah', $id_suratnikah);
		$this->db->update('upload_suratnikah', $data);
