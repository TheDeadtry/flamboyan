...
class New_majikans extends MX_Controller{
	...
	function index(){
		...
		$status = $session['session_status'];
			//user sudah login
			if ($id_user && $status==1){
				$data['sektor'] = $this->m_new_majikans->tampil_data_sektor();
	...
	function detail($id = ""){
		...
		$data['nama'] = $this->db->query("
			SELECT concat(kode_majikan,'-', nama) nama FROm datamajikan WHERE id_majikan = '$id'
		")->row()->nama;
		$data['datatki'] = $this->db->query("
		SELECT 
			p.id_biodata
			, p.nama
			, m.tglterbang
			, ifnull(ds.no_suhan,'-') no_suhan
			, ds.tglterima
			, ds.tglsimpan
			, m.statsuhan 
			, m.kirimsuhan
			, visa.ketdoksuhan
			, v.no_visapermit novisa
			, visa.tglterimadok
			, visa.statusterima
			, m.simpanvisapermit
			, m.kirimvisapermit
			, visa.ketdok
		FROM datamajikan d 
		JOIN majikan m ON d.id_majikan = m.kode_majikan
		LEFT JOIN datasuhan ds ON m.kode_majikan = ds.id_majikan AND m.kode_suhan = ds.id_suhan
		LEFT JOIN personal p ON m.id_biodata = p.id_biodata
		LEFT JOIN visa ON m.id_biodata = visa.id_biodata
		LEFT JOIN datavisapermit v ON ds.id_suhan = v.id_suhan
		WHERE d.id_majikan = '$id' GROUP BY p.id_biodata ORDER BY p.id_personal DESC, v.id_visapermit DESC
		")->result();
		echo Modules::run('template/new_admin_template', $data);
	}
	...
	function show_majikan() {
	    ...
	    foreach ($tampil_data_majikan as $row) {
			...
			if ($datafile != NULL) {
				$filemajikans_data = '<a href="'.base_url().'assets/dokmajikan/file-file/'.$datafile->file.'">'.$datafile->filetglinput.'</a>';
			} else {
            ...
            array_push($data2,
                array(
                    $no,
                    '<a style=" font-family: \'Roboto\', sans-serif;" href="'.site_url('new_majikans/detail/'.$row->id_majikan).'" class="btn btn-primary"><i class="fa fa-eye"></i> DATA TKI PERMAJIKAN</a>',
                    $filemajikans_data,
	...
	function show_suhan($kodemajikan, $kodeagen, $kodegroup) {
        ...
        foreach ($tampil_data_suhan as $row) {
        	if ($row->filesuhan != NULL) {
		        $imgs = '<a href="'.base_url().'assets/doksuhan/'.$row->filesuhan.'">';
