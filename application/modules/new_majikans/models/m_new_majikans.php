...
class m_new_majikans extends CI_Model{
    ...
    function tampil_data_majikan3($where, $limit) {
        $sql = "SELECT datamajikan.*,
        dataagen.nama as namaagen 
        FROM datamajikan 
        LEFT JOIN dataagen on datamajikan.kode_agen=dataagen.id_agen 
        $where 
        ORDER BY id_majikan DESC $limit";
    ...
    function update_majikan() {
        ...
        if(empty($_FILES['gambarnya']['name']))
        {
            $data = array(
                ...
                'datasektor_id' => $this->input->post('datasektor_id'),
                'namamajikan' => $this->input->post('namamajikan'),
                'pimpinan_man' => $this->input->post('pimpinan_man'),
                'pimpinan_indo' => $this->input->post('pimpinan_indo'),
                ...
                'alamat_mandarin' => $this->input->post('alamat_mandarin'),   
            );
            $this->db->where('id_majikan', $id);
    ...
    function tampil_data_sektor() {
        $sql = "SELECT id kode, nama text FROM sektormajikan";
        $query = $this->db->query($sql);
        return $query->result();
    } 
