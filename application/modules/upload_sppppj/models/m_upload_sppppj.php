...
class M_upload_sppppj extends CI_Model{
 ...
 function simpan_foto_sppppj($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-sppppj/';
	 ...
	 function hapus_foto_sppppj(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-sppppj/'.$nama_foto)){
				unlink($file);
			}
...
function tampil_data_personal($idnya){
	...
	function tampil_data_sppppj($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='sppppj'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_sppppj WHERE id_biodata='".$idnya."'");
