<?php
class M_upload_tiket extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_tiket($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-tiket/';
	 ...
	 function hapus_foto_tiket(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-tiket/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_tiket() {
        ...
        $config['upload_path'] = './assets/upload_tiket/'; //path folder
...
function tampil_data_personal($idnya){
	...
	function tampil_data_tiket($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='tiket'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_tiket WHERE id_biodata='".$idnya."'");
	...
	function update_data_tiket() {
		...
		$this->db->update('upload_tiket', $data);
