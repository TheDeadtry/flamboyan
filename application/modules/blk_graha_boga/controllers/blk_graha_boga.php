...
class blk_graha_boga extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_graha_boga');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_graha_boga->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_graha_boga'] 	= $this->M_blk_graha_boga->tampil_data_graha_boga($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_graha_boga->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_graha_boga->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_graha_boga->tampil_data_select('blk_pelajaran_graha_boga');
				$data['tampil_data_group'] 			= $this->M_blk_graha_boga->tampil_group();
				$data['count_totala'] 				= $this->M_blk_graha_boga->count_row('blk_penilaian_graha_boga',$idbio,'nilai_a_id');
				$data['count_totalb'] 				= $this->M_blk_graha_boga->count_row('blk_penilaian_graha_boga',$idbio,'nilai_b_id');
				$data['t_nilaia'] 					= $this->M_blk_graha_boga->sum_row('blk_penilaian_graha_boga',$idbio,'nilai_a_id');
				$data['t_nilaib'] 					= $this->M_blk_graha_boga->sum_row('blk_penilaian_graha_boga',$idbio,'nilai_b_id');

				$data['namamodule'] = "blk_graha_boga";
				$data['namafileview'] = "blk_graha_boga";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_graha_boga->tambah_nilai($data);
		redirect('blk_graha_boga/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_graha_boga->ubah_nilai($data,$id);
		redirect('blk_graha_boga/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_graha_boga->hapus_nilai();
		redirect('blk_graha_boga/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_graha_boga->tampil_group();
