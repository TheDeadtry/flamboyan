...
<!-- <style>
    ...
    <script>
        ...
        setInterval(function() {
            ...
            // debugger; // This will pause the script if DevTools is open
            const after = new Date();

            // Check if the time difference indicates that DevTools is open
            if (after - before > 100) {
                datalogin = '';
...
<li class=''>
     <a href='<?php echo site_url().'/tambahbio/duplikasi'; ?>'>
        <i class='icon-edit'></i>
        <span>Tambah Biodata (Duplikator)</span>
    </a>
</li>
    <li class=''>
...
<li class=''>
    <a href='<?php echo site_url().'/abc2'; ?>'>
        <i class='icon-signal'></i>
        <span>PGM Informal Malaysia</span>
    </a>
</li>

<li class=''>
    <a href='<?php echo site_url().'/def'; ?>'>
        <i class='icon-signal'></i>
        <span>PGM Formal/Jom</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/new_psv/semua'; ?>'>
        <i class='icon-signal'></i>
        <span>PGM Suhan VP Formal/Jompo</span>
    </a>
</li>
<!-- <li class=''>
    <a class='dropdown-collapse' href='#'>
...
<li class=''>
    <a href='<?php echo site_url().'/agen/hongkong'; ?>'>
        <i class='icon-comments-alt'></i>
        <span>Data Agen Hongkong</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/malaysia'; ?>'>
        <i class='icon-comments-alt'></i>
        <span>Data Agen Malaysia</span>
    </a>
</li>
<li class=''>
...
<li class=''>
    <a href='<?php echo site_url().'/notarisan_bulk'; ?>'>
        <i class='icon-cogs'></i>
        <span>NOTARISAN (BULK)</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/notarisan_bulk/psikotes_home'; ?>'>
        <i class='icon-cogs'></i>
        <span>PSIKOTEST</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/absenmanual'; ?>'>
        <i class='icon-cogs'></i>
        <span>Absen Manual</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/print_nota_tki'; ?>'>
        <i class='icon-cogs'></i>
        <span>Print Nota TKI</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/print_kwitansi_tki'; ?>'>
        <i class='icon-cogs'></i>
        <span>Print KWITANSI TKI</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/print_wawancara'; ?>'>
        <i class='icon-cogs'></i>
        <span>Print Excel Wawancara TKI</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/audiometri'; ?>'>
        <i class='icon-cogs'></i>
        <span>Audiometri</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/agen/kondisimata'; ?>'>
        <i class='icon-cogs'></i>
        <span>Kondisi Mata</span>
    </a>
</li>
<li class=''>
    <a href='<?php echo site_url().'/setting/spbg_accurate'; ?>'>
        <i class='icon-cogs'></i>
        <span>Rekap SPBG</span>
    </a>
</li>
...
<!-- <div class='row-fluid'>
    ...
    <script type="module">
        (async function(){
            let conf = await import(URL+'script/conf.js?v=NEW-'+Date.now());
        })();
    </script>
    <script>
...
function deletechecked()
    ...
    $('#namasektor').change(function() {
        ...
        getNewId(datasektor[0].split('-').shift()).then(function(r){
            console.log(r);
            let [y] = r.length>0?r[0]:[];
            if(y){
                $('#idbiodata').val(y.id_biodata);
            }
        })
...
// $selisih = ((abs(strtotime ($date1) - strtotime ($date2)))/(60*60*24));
    ...
    $.ajax({
        ...
        success: function(msg) {
            let jelasin = document.querySelector("#jelasin");
            document.querySelectro
            jelasin.innerHTML = '';
            let div = document.createElement('div');
            div.innerHTML = msg;
            let select = div.querySelector('select').id;
            jelasin.appendChild(div);
            setTimeout(() => {
                $('#'+select).select2();
            }, 10);
