...
class M_agen extends CI_Model{
 ...
 function simpan_data_agen(){
		...
		$data = array (
			...
			'noagree4'=> $this->input->post('noagree4'),
			'jenisagre4'=> $this->input->post('jenisagre4'),
			'berlaku4'=> $this->input->post('berlaku4'),
			'selesai4'=> $this->input->post('selesai4'),
			'tgl_terimaagree4'=> $this->input->post('tgl_terimaagree4'),
			
			'noagree5'=> $this->input->post('noagree5'),
			'jenisagre5'=> $this->input->post('jenisagre5'),
			'berlaku5'=> $this->input->post('berlaku5'),
			'selesai5'=> $this->input->post('selesai5'),
			'tgl_terimaagree5'=> $this->input->post('tgl_terimaagree5'),
  ...
  function simpandokumen(){
	...
	function tampil_data_agen(){
		$sql = "SELECT * FROM dataagen WHERE loc = 'taiwan'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT * FROM dataagen WHERE id_agen='$id_agen'");
...
$kode_desa="";
  ...
  function simpan_data_agree2(){
	...
	function tampil_data_agree4($id_agen){
		$sql = "SELECT * FROM agenagree4 WHERE id_agen='$id_agen' ORDER BY id_agree4 DESC";
                $query = $this->db->query($sql);

            return $query->result();
	}

	function tampil_data_agree5($id_agen){
		$sql = "SELECT * FROM agenagree5 WHERE id_agen='$id_agen' ORDER BY id_agree5 DESC";
                $query = $this->db->query($sql);
	...
	function tampil_jenis_agree4($id_agen){
		$kode_desa="";
        $result = Dbs::load("SELECT * FROM dataagen WHERE id_agen='$id_agen'");
		while($row = mysqli_fetch_array($result)){
			$kode_desa =$row['jenisagre4'];
		}
		return $kode_desa;
	} 

	function tampil_jenis_agree5($id_agen){
		$kode_desa="";
        $result = Dbs::load("SELECT * FROM dataagen WHERE id_agen='$id_agen'");
		while($row = mysqli_fetch_array($result)){
			$kode_desa =$row['jenisagre5'];
		}
		return $kode_desa;
	} 
  	...
  	function simpan_data_agree4(){

		$data = array (
			'id_agen'=> $this->input->post('id_agen'),
			'noagree4'=> $this->input->post('noagree'),
			'tglberlaku4'=> $this->input->post('berlaku'),
			'tglberakhir4'=> $this->input->post('selesai'),
			'tglterima4'=> $this->input->post('tgl_terimaagree'),
		);
		$this->db->insert('agenagree4',$data);
	}

  	function simpan_data_agree5(){

		$data = array (
			'id_agen'=> $this->input->post('id_agen'),
			'noagree5'=> $this->input->post('noagree'),
			'tglberlaku5'=> $this->input->post('berlaku'),
			'tglberakhir5'=> $this->input->post('selesai'),
			'tglterima5'=> $this->input->post('tgl_terimaagree'),
		);
		$this->db->insert('agenagree5',$data);
	}
	...
	function update_agree4() {
		$id = $this->input->post('id_agree4');
		$data = array(
			'noagree4'=> $this->input->post('noagree'),
			'tglberlaku4'=> $this->input->post('berlaku'),
			'tglberakhir4'=> $this->input->post('selesai'),
		...
		$this->db->where('id_agree4', $id);
		$this->db->update('agenagree4', $data);
	}
 	
	function update_agree5() {
		$id = $this->input->post('id_agree5');
		$data = array(
			'noagree5'=> $this->input->post('noagree'),
			'tglberlaku5'=> $this->input->post('berlaku'),
			'tglberakhir5'=> $this->input->post('selesai'),
		...
		$this->db->where('id_agree5', $id);
		$this->db->update('agenagree5', $data);
	}
	...
	function hapus_agree4() {
		$id = $this->input->post('id_agree4');
		$this->db->where('id_agree4', $id);
		$this->db->delete('agenagree4');
	}
	function hapus_agree5() {
		$id = $this->input->post('id_agree5');
		$this->db->where('id_agree5', $id);
		$this->db->delete('agenagree5');
	}
