        ...
        (async function(){
            ...
            const dataLoad = async function(num = 0, len = 50) {
                ...
                if (tableResult) {
                    ...
                    let ld = cssLoader();
                    let [total] = await dataGet(`SELECT count(*) total FROM kwitansi_spbg WHERE 1=1 ${fill} `);
                    let [dataRead] = await dataGet(`SELECT * FROM kwitansi_spbg WHERE 1=1 ${fill}  ORDER BY id DESC limit ${num},${len}`);
                    ld.remove();
                    let data = [];
                    // header
                    data.push({
