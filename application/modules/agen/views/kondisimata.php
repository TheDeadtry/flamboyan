<div class="row-fluid">
    <div class="span12">
        <div class="page-header">
            <h1 class="pull-left">
                <i class="icon-star"></i>
                <span id="title-page">Tes Kondisi Mata</span>
            </h1>
            <div class="pull-right">
                <ul class="breadcrumb">
                    ...
                    <li id="title-page2" class="active">Tes Kondisi Mata </li>
                </ul>
            </div>
        </div>
    </div>
        ...
        (async function(){
                ...
                for(let ac of action){
                    ac.addEventListener('click', function(){
                        ...
                        Delete(async function(ac){
                            let id = ac
                            let y = cssLoader();
                            await dataGet(`DELETE FROM kondisimata WHERE id = '${ac}'`);
                            y.remove();
                ...
                for(let ax of update){
                    ax.addEventListener('click', function(){
                        (async function(ac){
                            ...
                            let [data] = await dataGet(`SELECT keterangan FROM kondisimata WHERE id = '${pr}'`);
                            let [ket] = data
            ...
            const dataLoad = async function(num = 0, len = 50) {
                ...
                if (tableResult) {
                    ...
                    let ld = cssLoader();
                    let [total] = await dataGet(`SELECT count(*) total FROM kondisimata WHERE 1=1 ${fill} `);
                    let [dataRead] = await dataGet(`SELECT * FROM kondisimata WHERE 1=1 ${fill}  ORDER BY id DESC limit ${num},${len}`);
                    ld.remove();
                    let data = [];
                    // header
                    data.push({
            ...
            simpanTki.addEventListener('click', async function(){
                ...
                if(simpanData.length > 0){
                    let ldr = cssLoader();
                    await dataGet(simpanData.ToUpdate('kondisimata', ['id']));
                    dataLoad(0) 
                    ldr.remove();
                    addTki.modal('toggle')             
                }else{
            ...
            const formData = function(){
                ...
                let submitButton = el('button')
                    ...
                    .click(async function(e) {
                        ...
                        $.ajax({
                            url             : "<?php echo site_url('agen/simpankondisimata') ?>",
                            type            : "POST",
                            dataType        : 'json',
                            encode          : true,
                            data            : form_data,
