...
class M_upload_berkas extends CI_Model{
 ...
 function simpan_foto_berkas($idbio){
	 ...
	 function hapus_foto_berkas(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-berkas/'.$nama_foto)){
				unlink($file);
			}
...
function tampil_data_personal($idnya){
	...
	function tampil_data_berkas($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='berkas'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_berkas WHERE id_biodata='".$idnya."'");
