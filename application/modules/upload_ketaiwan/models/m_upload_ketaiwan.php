<?php
class M_upload_ketaiwan extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_ketaiwan($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-ketaiwan/';
	 ...
	 function hapus_foto_ketaiwan(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-ketaiwan/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_ketaiwan() {
        ...
        $config['upload_path'] = './assets/upload_ketaiwan/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_ketaiwan', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_ketaiwan($idnya){
		$sql = "SELECT * FROM upload_ketaiwan WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='ketaiwan'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_ketaiwan WHERE id_biodata='".$idnya."'");
	...
	function update_data_ketaiwan() {
		$id_ketaiwan =$this->input->post('id_ketaiwan');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_ketaiwan', $id_ketaiwan);
		$this->db->update('upload_ketaiwan', $data);
