<div class="row-fluid">
    <div class="span12">
        <div class="page-header">
            <h1 class="pull-left">
                <i class="icon-star"></i>
                <span id="title-page">CETAK PMI - TEST PSIKOLOGI</span>
            </h1>
            <div class="pull-right">
                <ul class="breadcrumb">
                    ...
                    <li id="title-page2" class="active">CETAK PMI - TEST PSIKOLOGI </li>
                </ul>
            </div>
        </div>
    </div>
        ...
        (async function(){
                ...
                if (tableResult) {
                    let fill = (function(){
                        ...
                        if(filterValue.nama && filterValue.nama != ''){
                            d += ` AND lower(nama_hk) LIKE lower('%${filterValue.nama}%') `
                        }
                        return d;
                    })();
                    let ld = cssLoader();
                    let [total] = await dataGet(`SELECT count(*) total FROM v_psikolog WHERE 1=1 ${fill} `);
                    let [dataRead] = await dataGet(`SELECT id, nama_hk nama, date_created, iq FROM v_psikolog WHERE 1=1 ${fill}  ORDER BY id DESC limit ${num},${len}`);
                    ld.remove();
                    let data = [];
                    // header
                    data.push({
                    ...
                    for(let dx of dataRead){
                        data.push({
                            option: `
                                <button class="btn btn-success edit" data-id="${dx.id}">Edit</button>
            ...
            const formData = function(){
                ...
                let submitButton = el('button')
                    ...
                    .click(async function(e) {
                        ...
                        $.ajax({
                            url             : "<?php echo site_url('notarisan_bulk/add_process') ?>",
                            type            : "POST",
                            dataType        : 'json',
                            encode          : true,
                            data            : form_data,
            ...
            const TableLoad = async function(){
                ...
                .child(
                    ...
                    .child(
                        el('p').css({fontSize:'18px', fontWeight:'bold', color:'gray'}).text('LIST PRINT PMI - TEST PSIKOLOGI')
                    )
                    .child(
                        el('div').css({
                            display : 'flex'
