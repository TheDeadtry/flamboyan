...
class notarisan_bulk extends MX_Controller{
	...
	public function show() {
		$columns22 = array(
            'a.id_biodata','b.id_biodata','b.nama'
		);
		...
		$query = "SELECT 
            ...
            b.nama
			from notarisan a
			LEFT JOIN personal_nama b ON a.id_biodata=b.id_biodata 
			$where_dd 
			order by tgl_nota desc 
			$limit
		";
	    ...
	    foreach ($tampil_data as $row) 
	    {
			...
			array_push($data2,
				array(
					$no,
					'<a class="btn btn-xs bg-teal" href="'.site_url('notarisan_bulk/uploaddokumen/'.$row->id_biodata).'">UPLOAD</a> '."\n".
        ...
        $recordsFiltered 	= $this->M_session->select_count("notarisan a
            JOIN personal_nama b ON a.id_biodata=b.id_biodata 
			$where_dd
		");
	...
	public function printformprocess() {
		...
		if ($type == 'formal') {
			...
			$mapfilter = array_map(function($h){
				return ' a.id_biodata LIKE "'.$h.'-%"';
			}, explode(",", $type) );
			$where = 'WHERE ('.join(" OR ", $mapfilter).') && tgl_nota="'.$tgl.'" ';
		}
	...
	public function belum($id="") {
		...
		LEFT JOIN notarisan ON personal.id_biodata=notarisan.id_biodata
		WHERE notarisan.id_biodata IS NULL 
		AND lower( statusaktif ) <> 'mengundurkan diri'
		AND lower( statusaktif ) <> 'unfit'
		AND lower( statusaktif ) <> 'pending'
		AND lower( statusaktif ) NOT LIKE '%pindah%'
		AND lower( statusaktif ) NOT LIKE '%ptm%'
        ORDER BY personal.id_biodata ASC");
		$data['namamodule'] 	= "notarisan_bulk";
	...
	public function psikotes($id="") {
		$data['tampil_data_pmi'] = $this->M_session->select("SELECT personal.nama,personal.id_biodata FROM personal_nama personal 
		WHERE 1=1
		AND notaris_count IS NULL
		AND lower( statusaktif ) <> 'mengundurkan diri'
		AND lower( statusaktif ) <> 'unfit'
		AND lower( statusaktif ) <> 'pending'
		AND lower( statusaktif ) NOT LIKE '%pindah%'
        ...
        ORDER BY personal.id_biodata ASC");
		$data['namamodule'] 	= "notarisan_bulk";
		$data['namafileview'] 	= "psikotes";
		echo Modules::run('template/new_admin_template',$data);
	}

	function psikotes_home(){
		$data['namamodule'] = "notarisan_bulk";
		$data['namafileview'] = "psikoteshome";
	...
	function add_process() 
    {
		...
		foreach($pmi as $id){
			...
			$kas = $this->M_session->insert_return_id($data2, 'blk_psikolog_nilai');
		}
		...
		$this->db->query("DELETE FROM blk_psikolog WHERE id_biodata = ''");
	...
	public function belumdata() {
		...
		$pop = "SELECT personal.nama,personal.id_biodata FROM personal 
		LEFT JOIN notarisan ON personal.id_biodata=notarisan.id_biodata
		WHERE notarisan.id_biodata IS NULL 
		AND lower( statusaktif ) <> 'mengundurkan diri'
		AND lower( statusaktif ) <> 'unfit'
		AND lower( statusaktif ) <> 'pending'
		AND lower( statusaktif ) <> 'sudah terbang'
		AND lower( statusaktif ) NOT LIKE '%pindah%'
		AND lower( statusaktif ) NOT LIKE '%ptm%'
	...
	function cetaknotaris($sektor, $date1="", $date2="", $nama=""){
		...
		$document = $PHPWord->loadTemplate('files/notaris/notaris.docx');

		$filename = 'belum notaris.docx';
		$this->load->model('Mpk');
	...
	function cetakpsikotes($sektor, $date1="", $date2="", $nama=""){
		require_once 'assets/phpword/PHPWord.php';
		$PHPWord = new PHPWord();
