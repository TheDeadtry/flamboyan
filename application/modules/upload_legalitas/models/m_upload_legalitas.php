<?php
class M_upload_legalitas extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_legalitas($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-legalitas/';
	 ...
	 function hapus_foto_legalitas(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-legalitas/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_legalitas() {
        ...
        $config['upload_path'] = './assets/upload_legalitas/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_legalitas', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_legalitas($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='legalitas'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_legalitas WHERE id_biodata='".$idnya."'");
	...
	function update_data_legalitas() {
		...
		$this->db->update('upload_legalitas', $data);
