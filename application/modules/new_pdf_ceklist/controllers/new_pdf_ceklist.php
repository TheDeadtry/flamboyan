...
class new_pdf_ceklist extends MX_Controller{
	...
	function ceklist($id_pembuatan=""){
		...
		$datadisnaker = $this->m_printdata->ambildisnakernya($idtkinya)->row();
		...
		$document = $PHPWord->loadTemplate('files/disnaker/dl004_baru.docx');
		...
		$this->load->model('Todocx');
		
		$this->Todocx->datapk([
		  "disnaker" => $datadisnaker,
		  "eks" => $eksnya,
		  "id" => $idtkinya,
		], $document);
	...
	function ceklist_bc($id_pembuatan) 
	{
...
$tgl5 = date("d-m-Y", strtotime($originalDate5));
	...
	$new_alamat = $new_alamatkontak = substr($d, $e1);
