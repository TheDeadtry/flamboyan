<?php
class M_upload_desuhan extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_desuhan($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-desuhan/';
	 ...
	 function hapus_foto_desuhan(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-desuhan/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_desuhan() {
        ...
        $config['upload_path'] = './assets/upload_desuhan/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_desuhan', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_desuhan($idnya){
		$sql = "SELECT * FROM upload_desuhan WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='desuhan'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_desuhan WHERE id_biodata='".$idnya."'");
	...
	function update_data_desuhan() {
		$id_desuhan =$this->input->post('id_desuhan');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_desuhan', $id_desuhan);
		$this->db->update('upload_desuhan', $data);
