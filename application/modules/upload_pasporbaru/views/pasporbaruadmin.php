                                            ...
                                            <td><?php echo $row->penting;?></td>
                                             <td><?php echo $row->cekdokumen;?></td>
                                              <td><?php echo $row->tglterima;?></td>
                                               ...
                                               <td>
                                                    <a href="<?php echo base_url().'/assets/uploadpasporbaru/'.$row->file;?>"<?php echo $row->file;?>><img src="<?php echo base_url(); ?>assets/uploadpasporbaru/<?php echo "".$row->file ?>"/></a> </td>
...
<div class='modal hide fade' id='tambah' role='dialog' tabindex='-2'>
                <div class='modal-header'>
                    <button class='close' data-dismiss='modal' type='button'>&times;</button>
                    <h3>Tambah Paspor Baru</h3>
                </div>
                <div class='modal-body'>
<form id="simpandata" action="<?php echo site_url('upload_pasporbaru/simpan_data_pasporbaru');?>" enctype="multipart/form-data" method="post" class="form-horizontal" />
...
<script>
    ...
    function blobToWebpFile(blobData, fileName) {
        // Buat objek File dari objek Blob
        const fileOptions = { type: 'image/webp', lastModified: Date.now() }; // Opsi tambahan file
        const file = new File([blobData], fileName, fileOptions);
        
        return file;
    }

    function arrayToFormData(arrayData) {
        const formData = new FormData();
        
        // Iterasi melalui setiap elemen array dan tambahkan ke FormData
    ...
    $("#simpandata").on("submit", function(e){
        ...
        const dat = dataArray;
        const image = dataArray.pop();
        console.log(image.value.name);
        let ll = cssLoader();
        if(image.value.name && image.value.name != ''){
            (async function(){
                const ff = await convertToWebP(image.value, (2048 * 2), (2048 * 2));
                const file = blobToWebpFile(ff,'Paspor '+Date.now()+'.webp');
                if(file){
                    dataArray.push({name:image.name, value: file});
                }
                const formData = arrayToFormData(dataArray)
                let link = e.target.action.replace(/http:/gi,'https:');
            ...
            (async function(){
                console.log(dat);
                const formData = arrayToFormData(dat)
                let link = e.target.action.replace(/http:/gi,'https:');
                fetch(link, {
                    method: 'POST',
                    body: formData
