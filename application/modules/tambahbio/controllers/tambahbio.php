...
class Tambahbio extends MX_Controller{
...
if (isset($_POST['resetid'])) {
	...
	function duplikasiSimpanProcess($baru, $lama) {
		$idBaru = $baru;
		$idLama = $lama;
		...
		foreach ($data3 as $v) {
			...
			$datax3 = $this->M_session->insert($v, "pengalaman");
		}
	...
	function dup(){
		$data = [
			 "id_biodata" => "TEST-0001" 
			 ,"optIdLama" => "FF-0198"
		];
		$this->duplikasiSimpanProcess($data['id_biodata'], $data['optIdLama']);
	}
