...
<?php
class M_upload_pasportampil extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_pasportampil($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-pasportampil/';
	 ...
	 function hapus_foto_pasportampil(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-pasportampil/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_pasportampil() {
        ...
        $config['upload_path'] = './assets/upload_pasportampil/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_pasportampil', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_pasportampil($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='pasportampil'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_pasportampil WHERE id_biodata='".$idnya."'");
	...
	function update_data_pasportampil() {
		$id_pasportampil =$this->input->post('id_pasportampil');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_pasportampil', $id_pasportampil);
		$this->db->update('upload_pasportampil', $data);
