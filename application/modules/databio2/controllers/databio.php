...
*/
		...
		if ($status == 1) {
			...
			$final_fulliest = 'SELECT * FROM personal where '.$fullfinss.' '.$idbio_query;
			...
			$fullfinss = $status_half.' '.$fspons.' '.$tgldaft;
			$final_fulliest = 'SELECT a.* FROM personal a JOIN majikan b ON a.id_biodata=b.id_biodata where '.$fullfinss.' a.'.$idbio_query.' and b.tglterpilih IS NULL and b.tglterpilih != 0';
			...
			$fullfinss = $status_half.' '.$fspons.' '.$tgldaft;
			$final_fulliest = 'SELECT a.* FROM personal a JOIN majikan b ON a.id_biodata=b.id_biodata where '.$fullfinss.' a.'.$idbio_query.' and b.tglterpilih IS NOT NULL';

		} elseif ($status == 13) {
			$status_half = "";
			$status_word = "SEMUA";
			$fullfinss = $status_half.' '.$fspons.' '.$tgldaft;
			$final_fulliest = 'SELECT * FROM personal where '.$fullfinss.' id_biodata like "'.$idbioz.'%"';
		...
		foreach ($zselectionz as $value) {
			...
			$xxc_tgldaftar = 'SELECT tanggaldaftar,statusaktif,tgllahir,tinggi,berat,pendidikan,notelp,notelpkel,ketadm,ketdok FROM personal where id_biodata ="'.$value->id_biodata.'"';
...
/*		
		    ...
		    */
		    ...
		    $document->setValue('value4#'.$nn,$f_s_spon);
		    ...
		    $document->setValue('value8#'.$nn,'/'.$ftinggi[$nn].'/'.$fberat[$nn]);
