<?php
class M_upload_sikb extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_sikb($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-sikb/';
	 ...
	 function hapus_foto_sikb(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-sikb/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_sikb() {
        ...
        $config['upload_path'] = './assets/upload_sikb/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_sikb', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_sikb($idnya){
		$sql = "SELECT * FROM upload_sikb WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='sikb'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_sikb WHERE id_biodata='".$idnya."'");
	...
	function update_data_sikb() {
		$id_sikb =$this->input->post('id_sikb');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_sikb', $id_sikb);
		$this->db->update('upload_sikb', $data);
