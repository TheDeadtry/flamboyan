...
class blk_mandarin_pabrik extends MX_Controller{
	public function __construct(){
            parent::__construct();
			$this->load->model('M_session');			
			$this->load->model('M_blk_mandarin_pabrik');			
	}
		...
		$status = $session['session_status'];
			//user sudah login
			 if ($id_user && $status==2){
				...
				$tampil_data_blk_personaldetail = $this->M_blk_mandarin_pabrik->tampil_data_blk_personal($pilihan);
				$data['namanya'] = $tampil_data_blk_personaldetail['namanya'];

			 	//$data['tampil_data_mandarin_pabrik'] 	= $this->M_blk_mandarin_pabrik->tampil_data_mandarin_pabrik($idbio);
			 	$data['tampil_data_blk_instruktur'] = $this->M_blk_mandarin_pabrik->tampil_data_select('blk_instruktur');
			 	$data['tampil_data_blk_nilai'] 		= $this->M_blk_mandarin_pabrik->tampil_data_select('blk_nilai');
			 	$data['tampil_data_blk_materi'] 	= $this->M_blk_mandarin_pabrik->tampil_data_select('blk_pelajaran_mandarin_pabrik');
				$data['tampil_data_group'] 			= $this->M_blk_mandarin_pabrik->tampil_group();
				$data['count_totala'] 				= $this->M_blk_mandarin_pabrik->count_row('blk_penilaian_mandarin_pabrik',$idbio,'nilai_a_id');
				$data['count_totalb'] 				= $this->M_blk_mandarin_pabrik->count_row('blk_penilaian_mandarin_pabrik',$idbio,'nilai_b_id');
				$data['t_nilaia'] 					= $this->M_blk_mandarin_pabrik->sum_row('blk_penilaian_mandarin_pabrik',$idbio,'nilai_a_id');
				$data['t_nilaib'] 					= $this->M_blk_mandarin_pabrik->sum_row('blk_penilaian_mandarin_pabrik',$idbio,'nilai_b_id');

				$data['namamodule'] = "blk_mandarin_pabrik";
				$data['namafileview'] = "blk_mandarin_pabrik";
	...
	function tambah_nilai($no) {
		...
		$this->M_blk_mandarin_pabrik->tambah_nilai($data);
		redirect('blk_mandarin_pabrik/index/'.$no);
	}
	...
	function ubah_nilai($no) {
		...
		$this->M_blk_mandarin_pabrik->ubah_nilai($data,$id);
		redirect('blk_mandarin_pabrik/index/'.$no);
	}

	function hapus_nilai($no) {
		$this->M_blk_mandarin_pabrik->hapus_nilai();
		redirect('blk_mandarin_pabrik/index/'.$no);
	}
	...
	function coba($idbio) {
		...
		$tampil_data_group 			= $this->M_blk_mandarin_pabrik->tampil_group();
