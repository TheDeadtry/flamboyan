...
class M_upload_pasporlama extends CI_Model{
 ...
 function simpan_foto_pasporlama($idbio){
	...
	function simpan_data_pasporlama() {
        ...
        $config['upload_path'] = './assets/upload_pasporlama/'; //path folder
...
function tampil_data_personal($idnya){
	...
	function tampil_data_pasporlama($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='pasporlama'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_pasporlama WHERE id_biodata='".$idnya."'");
