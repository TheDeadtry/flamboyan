...
class Print_expire_tgl_online extends MX_Controller{
	...
	function index() {	
		$data['namamodule'] = "print_expire_tgl_online";
		$data['namafileview'] = "print_expire_tgl_online";
		echo Modules::run('template/new_admin_template', $data);
	}


	function ambildata($dateone = "", $datetwo = ""){
		...
		$exprsion = "";
		if ($date1 != "" && $date2 != "") {
			$exprsion = "AND (DATE_ADD(disnaker.tglonline, INTERVAL 6 MONTH) BETWEEN '".$date1."' AND '".$date2."')";
		}
		...
		$sql = "
			SELECT DISTINCT
				...
				disnaker.tglonline,
				DATE_ADD(disnaker.tglonline,INTERVAL 6 MONTH) AS expired
			FROM
				personal
			...
			AND personal.statusaktif = 'SUDAH ADA ID'
			".$exprsion."
ORDER BY expired ASC
	...
	function printdata(){
		...
		if (isset($_POST["date01"]) && isset($_POST["date02"])) {
			$date1 = $_POST["date01"];
			$date2 = $_POST["date02"];
		}
...
ORDER BY expired ASC
		...
		foreach ($dataprint as $key => $value) {
			$isidata .='
				<tr>
					...
					<th colspan="8">'.$value->nodisnaker.'</th>
					<th colspan="8">'.$value->tglonline.'</th>
    	...
    	$html = '<table align="center" width="100%" cellspacing="0" cellpadding="2" border="0">
					<tr>
						<th><b> DAFTAR EXPIRED TANGGAL ONLINE BELUM TERBANG (SUDAH ID)</b></th>   
					</tr>
					<tr>
				...
				<table align="center" width="100%" cellspacing="0" cellpadding="2" border="0.1em">
						<tr>
							<th colspan="2" > NO</th>  
							<th colspan="8" > No Disnaker </th> 
							<th colspan="8" > Tanggal Online </th> 
