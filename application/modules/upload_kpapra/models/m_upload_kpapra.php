<?php
class M_upload_kpapra extends CI_Model{
    function __construct(){
        parent::__construct();
    }
// KPA pra == JD
 function simpan_foto_kpapra($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-kpapra/';
		if(!file_exists($config['upload_path'])){
			mkdir($config['upload_path'],0777,true);
		}
        $config['allowed_types'] = 'gif|jpg|png|jpeg|bmp|doc|docx|pdf|xls|xlsx|pdf';
	 ...
	 function hapus_foto_kpapra(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-kpapra/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_kpapra() {
        ...
        $config['upload_path'] = './assets/upload_kpapra/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_kpapra', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_kpapra($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='kpapra'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_kpapra WHERE id_biodata='".$idnya."'");
	...
	function update_data_kpapra() {
		$id_kpapra =$this->input->post('id_kpapra');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_kpapra', $id_kpapra);
		$this->db->update('upload_kpapra', $data);
