...
class new_pdf_pp extends MX_Controller{
...
*/
  ...
  public function zerocost_banyuwangi($id_penempatan, $cabang="", $khusus = "")
  {
	...
	*/
	...
	public function tambahandatapp($document, $id_penempatan){
		$disnaker = $this->db->query("SELECT disnaker.*, datanamadisnaker.namadisnaker as namadisnakernama FROM disnaker LEFT JOIN datanamadisnaker ON disnaker.namadisnaker_id=datanamadisnaker.id_namadisnaker WHERE id_biodata='$id_penempatan'")->row();
		$perjanjian = $this->db->query("SELECT * FROM pembuatan_perjanjian WHERE id_tki='$id_penempatan'")->row();

		$document->setValue('ktpno', $disnaker->noktp?$disnaker->noktp:'-');
        $document->setValue('jk', $disnaker->jeniskelamin?$disnaker->jeniskelamin:'-');
		
		function formatTanggal($data=""){
			return date('d-m-Y', strtotime( str_replace(".","-",$data) ) );
		}

        $document->setValue('ktptgl', $disnaker->tglnoktp? formatTanggal($disnaker->tglnoktp) :'-');
        $document->setValue('ktpkluar', $disnaker->tempatnoktp?$disnaker->tempatnoktp:'-');
        $document->setValue('pend', $disnaker->pendidikan?$disnaker->pendidikan:'-');
        $document->setValue('walialamat', $disnaker->alamatkontak?$disnaker->alamatkontak:'-');
        $document->setValue('gaji', $perjanjian->a7?$perjanjian->a7:'-');
	}
    ...
    public function novformal($id_penempatan){
        ...
        $PHPWord = new PHPWord();
		$sektor = explode("-",$id_penempatan)[0];
		if($sektor == "IM"){
			$document = $PHPWord->loadTemplate('files/pp/malaysia.docx');
		}else if($sektor == "HK"){
			$document = $PHPWord->loadTemplate('files/pp/hongkong.docx');
		}else{
			$document = $PHPWord->loadTemplate('files/pp/formal.docx');
		}
...
//print_r($bapak);
        ...
        $this->tambahandatapp($document,$id_penempatan);
    ...
    public function novinformal($id_penempatan){
		...
		if($sektor == "IM"){
			...
			$document = $PHPWord->loadTemplate('files/pp/informal.docx');
		}
