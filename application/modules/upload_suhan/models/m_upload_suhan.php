<?php
class M_upload_suhan extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_suhan($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-suhan/';
	 ...
	 function hapus_foto_suhan(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-suhan/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_suhan() {
        ...
        $config['upload_path'] = './assets/upload_suhan/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_suhan', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_suhan($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='suhan'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_suhan WHERE id_biodata='".$idnya."'");
	...
	function update_data_suhan() {
		$id_suhan =$this->input->post('id_suhan');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_suhan', $id_suhan);
		$this->db->update('upload_suhan', $data);
