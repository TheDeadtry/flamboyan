<?php
class M_upload_fotovisa extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_fotovisa($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-fotovisa/';
	 ...
	 function hapus_foto_fotovisa(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-fotovisa/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_fotovisa() {
        ...
        $config['upload_path'] = './assets/upload_fotovisa/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_fotovisa', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_fotovisa($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='fotovisa'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_fotovisa WHERE id_biodata='".$idnya."'");
	...
	function update_data_fotovisa() {
		$id_fotovisa =$this->input->post('id_fotovisa');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_fotovisa', $id_fotovisa);
		$this->db->update('upload_fotovisa', $data);
