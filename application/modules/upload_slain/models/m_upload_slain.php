<?php
class M_upload_slain extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_slain($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-slain/';
	 ...
	 function hapus_foto_slain(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-slain/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_slain() {
        ...
        $config['upload_path'] = './assets/upload_slain/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_slain', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_slain($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='slain'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_slain WHERE id_biodata='".$idnya."'");
	...
	function update_data_slain() {
		$id_slain =$this->input->post('id_slain');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_slain', $id_slain);
		$this->db->update('upload_slain', $data);
