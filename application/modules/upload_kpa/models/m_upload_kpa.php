<?php
class M_upload_kpa extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_kpa($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-kpa/';
	 ...
	 function hapus_foto_kpa(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-kpa/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_kpa() {
        ...
        $config['upload_path'] = './assets/upload_kpa/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_kpa', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_kpa($idnya){
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='kpa'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_kpa WHERE id_biodata='".$idnya."'");
	...
	function update_data_kpa() {
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_kpa', $id_kpa);
		$this->db->update('upload_kpa', $data);
