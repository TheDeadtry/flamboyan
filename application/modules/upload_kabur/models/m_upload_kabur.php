<?php
class M_upload_kabur extends CI_Model{
    function __construct(){
        parent::__construct();
    }

 function simpan_foto_kabur($idbio){

 		$nmfile =time().$idbio; //nama file saya beri nama langsung dan diikuti fu
        $config['upload_path']   = FCPATH.'/upload-kabur/';
	 ...
	 function hapus_foto_kabur(){
		...
		if($foto->num_rows()>0){
			...
			if(file_exists($file=FCPATH.'/upload-kabur/'.$nama_foto)){
				unlink($file);
			}
	...
	function simpan_data_kabur() {
        ...
        $config['upload_path'] = './assets/upload_kabur/'; //path folder
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->insert('upload_kabur', $data);
...
function tampil_data_personal($idnya){
	...
	function tampil_data_kabur($idnya){
		$sql = "SELECT * FROM upload_kabur WHERE id_biodata='".$idnya."'";
                $query = $this->db->query($sql);
		...
		$sql = "SELECT * FROM foto WHERE id_biodata='".$idnya."' AND jenis='kabur'";
                $query = $this->db->query($sql);
...
$kode_desa="";
        $result = Dbs::load("SELECT count(*) as hitungan FROM upload_kabur WHERE id_biodata='".$idnya."'");
	...
	function update_data_kabur() {
		$id_kabur =$this->input->post('id_kabur');
		...
		if(empty($_FILES['filenya']['name']))
        {
			...
			$this->db->where('id_kabur', $id_kabur);
		$this->db->update('upload_kabur', $data);
