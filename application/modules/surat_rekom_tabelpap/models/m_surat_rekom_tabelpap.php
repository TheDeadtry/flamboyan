...
$daerah = $this->input->post('daerah');
         ...
         $id2=$this->input->post('id2');
          $id3=$this->input->post('id3');
           $id4=$this->input->post('id4');
            ...
            $idsimpan=$id1."".$id2."".$id3."".$id4."".$id5."".$id6."".$id7;
    ...
    function tampil_data_all_sektor(){
     ...
     function alltki(){
        ...
        $sql = "SELECT personal.id_biodata, personal.nama FROM personal_nama personal
            LEFT JOIN paspor
            ON paspor.id_biodata=personal.id_biodata 
            WHERE 1=1
            AND paspor.keterangan='sudah' 
     ...
     function tampil_data_ff(){
        $sql = "SELECT * FROM personal_nama personal
                LEFT JOIN paspor
                ON paspor.id_biodata=personal.id_biodata 
                WHERE personal.id_biodata LIKE 'ff%' 
                AND paspor.keterangan='sudah' 
     ...
     function tampil_data_fh(){
        $sql = "SELECT * FROM personal_nama personal
                LEFT JOIN paspor
                ON paspor.id_biodata=personal.id_biodata 
                WHERE 
                (personal.id_biodata LIKE 'fh%' OR personal.id_biodata LIKE 'mh%') 
                AND paspor.keterangan='sudah' 
                order by personal.id_biodata ASC
                ";
                $query = $this->db->query($sql);
     ...
     function tampil_data_mc(){
        $sql = "SELECT * FROM personal_nama personal
                LEFT JOIN paspor
                ON paspor.id_biodata=personal.id_biodata 
                WHERE personal.id_biodata LIKE 'mc%' 
                AND paspor.keterangan='sudah' 
                order by personal.id_biodata ASC
                ";
                $query = $this->db->query($sql);
