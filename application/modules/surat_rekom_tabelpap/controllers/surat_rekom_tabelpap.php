...
class surat_rekom_tabelpap extends MX_Controller{
	...
	function fh(){
		$data = $this->M_surat_rekom_tabelpap->tampil_data_mc();
		var_dump($data);
	}
	...
	function detailtabelpap($idtabelpap){
		...
		$data['tampil_data_mc'] = $this->M_surat_rekom_tabelpap->tampil_data_mc();
		$data['tampil_data_fh'] = $this->M_surat_rekom_tabelpap->tampil_data_fh();
	...
	function detailtabelpap2($idtabelpap){
		$data['tampil_data_detail'] = $this->M_surat_rekom_tabelpap->tampil_data_detail($idtabelpap);
		
		$data['alltki'] = $this->M_surat_rekom_tabelpap->alltki();
		// $data['tampil_data_mc'] = $this->M_surat_rekom_tabelpap->tampil_data_mc();
		// $data['tampil_data_fh'] = $this->M_surat_rekom_tabelpap->tampil_data_fh();
		// $data['tampil_data_fi'] = $this->M_surat_rekom_tabelpap->tampil_data_fi();
		// $data['tampil_data_mf'] = $this->M_surat_rekom_tabelpap->tampil_data_mf();
		...
		$data['id_pembuatan'] = $idtabelpap;
		$data['namamodule'] = "surat_rekom_tabelpap";
		$data['namafileview'] = "detailtabelpap2";
	    ...
	    foreach ($tampil_data_pengajuan as $row) {
            ...
            array_push($data2,
                array(
                    ...
                    '
                        <a href="'.base_url().'index.php/pdf13/cetakppad/'.$row->id_pembuatanpap.'"  class="btn btn-warning">Print</a>
                        <a href="'.base_url().'index.php/pdf13/cetakopp/'.$row->id_pembuatanpap.'"  class="btn btn-warning">Print OPP</a>
                    ',
                    '<a href="'.site_url('surat_rekom_tabelpap/detailtabelpap2/'.$row->id_pembuatanpap).'" class="btn btn-primary" type="button">Tampil CTKI</a>',
                    $row->nomor,
                    $row->nomorktkln,
                    $row->kepada,
                    $row->daerah,
	    ...
	    foreach ($tampil_data_detail as $row) {
            ...
            array_push($data2,
                array(
		            ...
		            <a href="#" class="btn btn-mini btn-primary" data-toggle="modal" onClick=hapus999("'.$row->id_pembuatan.'") data-target="#hapus'.$no.'"><span>Hapus</span></td>',
		            $row->id,
		            $row->nama
                )
            );
   	...
   	function edit_detail_show() {
		...
		$querys = "
			SELECT 
				dt.id_pembuatan
				, dt.id_biodata 
				, concat(dt.id_biodata,\" \",p.nama) nama
			FROM detail_tabelpap dt 
			LEFT JOIN personal_nama p ON dt.id_biodata = p.id_biodata
			where dt.id_pembuatan='$id'
		";
		// echo $querys;
		$data = $this->M_surat_rekom_tabelpap->select_row($querys);
