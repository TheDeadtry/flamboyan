...
class Datatablegugus extends MX_Controller
{
    private $nama_table;
    private $limit_start;
    private $limit_end;
    ...
    public function location_view($aa){
        $this->datatableView = $aa;
    }
    ...
    public function set_delete_query($aa)
    {
        return $this->delete_query_table = "DELETE FROM ".$this->nama_table." WHERE ".$aa;
    }

    public function delete_data($aa)
    {
        return $this->db->query($this->delete_query_table);
    }
    ...
    private function buat_table(){
        ...
        foreach($theTable as $key => $value){
            ...
            if($this->edit_show == true){
                $child[] = "
                <center>
                    <a href='".$this->datatableView.'/'.$value->$key_id."' class='btn btn-info View'>View</a>
    ...
    // {
    //     $datarealaeseupdate = $this->db->query("SELECT * FROM ".$this->nama_table." WHERE ".$this->keys." = '".$this->keyword_update."'")->row();
